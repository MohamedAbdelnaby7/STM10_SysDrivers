...
void MEXTI_voidInit()
{
	/*configuaring default external interrupt EXTI0 Rising edge*/
...
{
	/*configuring EXTI on line Copy_u8Line with sensing mode Copy_u8SenseMode*/
...
{
	/*set functions to call in the app layer when interrupt is fired*/
	switch(Copy_u8Line)
	{
		case LINE0:
			EXTI_CallBack[0] = ptr;
...
{
	/*call function in EXTI_CallBack[0] when EXTI0 is fired*/
	EXTI_CallBack[0]();
	SET_BIT(EXTI -> PR, 0);
}
